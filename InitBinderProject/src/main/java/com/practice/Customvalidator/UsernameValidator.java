...
//this class need to register in side init Method....
...
//		Validating to not allow the Empty os null
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "name", "username.Empty", "Name Should not be Empty");
		
//		Validating to not allow "_" which contains "_" 
		BuildDTO username=(BuildDTO)target;
		 if(!username.getName().contains("_")) {
			 errors.rejectValue("name", "username.Invalide", "username must contains '_' ");
		 }
